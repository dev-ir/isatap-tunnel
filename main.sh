...
DVHOST_CLOUD_TUNNEL(){
    ...
    case $choice in
        1)
            setup_tunnel_and_forward
        ;;
        2)
            cleanup_socat_tunnel
        ;;
        0)
            echo -e "${GREEN}Exiting program...${NC}"
            exit 0
...
check_status() {
    if ip link show isatap1 &> /dev/null; then
        echo -e "\e[32mTunnel is UP.\e[0m"  # سبز: تونل برقرار است
    else
        echo -e "\e[31mTunnel is DOWN.\e[0m"  # قرمز: تونل برقرار نیست
    fi
}
...
DVHOST_CLOUD_GET_LOCAL_IP(){
    ...
    case $choice in
        1)
            ...
            create_tunnel_and_ping $server1_ip $server2_ip
        ...
        2)
            ...
            create_tunnel_and_ping $server2_ip $server1_ip
...
create_tunnel_and_ping() {
    local this_server_ip=$1
    local remote_server_ip=$2
    local this_server_hex=$(ip_to_hex $this_server_ip)
    local remote_server_hex=$(ip_to_hex $remote_server_ip)
    ...
    echo "=========================================="
    # echo "| IPv6 address of this server: $this_server_ipv6 |"
